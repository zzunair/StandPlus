...
(function($){
  ...
  if(Shopify && typeof Shopify === 'object') {
      ...
      Shopify.formatMoney = function (cents, format) {
          ...
          function formatWithDelimiters(number, precision, thousands, decimal) {
              ...
              var centsAmount = parts[1] && parts[1] !='00' ? decimal + parts[1] : '';
            //   console.log("centsAmount:", centsAmount);
  ...
  Popups.prototype = $.extend({}, Popups.prototype, {
      ...
      closeByName: function(name, $target, callback) {
        ...
        // console.log("in closeByName:", name);
          if($content.length) {
  ...
  StoreListsEngine.prototype = $.extend({}, StoreListsEngine.prototype, {
          ...
          $body.on('click', this.selectors.button, function(e) {
              ...
              if($this.attr('data-button-status') === 'added') {
                  _.removeItem(id, handle, function(data) {
                    const savedStatus = $this.attr('data-button-status-saved');
                    // console.log("savedStatus:", savedStatus);
                    if (savedStatus) {
                        $this.attr('data-button-status', savedStatus)
                    } else {
                  ...
                  _.addItem(id, handle, function(data) {
                      const saveStatus = $this.attr('data-button-status');
                    //   console.log("saveStatus:", saveStatus);
                      if (saveStatus) {
                        $this.attr('data-button-status-saved', saveStatus)
                      }
      ...
      getCustomerList: function(callback) {
          if(theme.customer && theme.lists_app?.url) {
              $.ajax({
                  type: 'POST',
      ...
      checkProductStatus: function(products) {
        ...
        //   console.log("checkProductStatus:", $active_products.find(_.selectors.button).attr('data-button-status'));
  ...
  function ProductOptions() {
      ...
      this.afterChange = [];
      // console.log("ProductOptions called");
      this.load();
  };
  ...
  ProductOptions.prototype = $.extend({}, ProductOptions.prototype, {
      load: function() {
          ...
          function onProcess(e, that) {
              var $this = that || $(this),
                ...
                debug && console.log("onProcess: e:", e);
                debug && console.log("onProcess: that:", that);
                debug && console.log("onProcess: this:", $this);
                debug && console.log("onProcess: options:", $options);
                debug && console.log("onProcess: section:", $section);
            ...
            //   }
              if ($this.hasClass('coming_soon')) {
                $options.find('.option-size .coming_soon').removeClass('d-none');
                window._klOnsite = window._klOnsite || [];
                window._klOnsite.push(['openForm', 'TdkHLV']);
              } else {
                $options.find('.option-size .coming_soon').addClass('d-none');
              }
            
              var $values = $section.find('[data-js-option-value]'),
                  $product = $this.parents('[data-js-product]'),
                  $variant_descriptions = $product.find('[data-option1]'),
                  json = $product.attr('data-json-product'),
                  current_values = [],
                  ...
                  debug && console.log("onProcess: values:", $values);
                  debug && console.log("onProcess: product:", $product);
                  debug && console.log("onProcess: variant_descriptions:", $variant_descriptions);
                  debug && console.log("onProcess: product json:", json);
              ...
              debug && console.log("data-value:", data_value);
              ...
              _._loadJSON($product, json, function (json) {
                  ...
                  current_values_string = current_values.join(' / ');
                  debug && console.log("onProcess: current_values:", current_values_string);
                      
                  $.each(json?.variants, function() {
                      if(current_values.join(' / ') === Shopify.handleizeArray(this.options).join(' / ')) {
                        debug && console.log("onProcess: shopify options:", Shopify.handleizeArray(this.options).join(' / '));
                  ...
                  debug && console.log("onProcess: json:", json);
                  handle = json.handle;
                  debug && console.log("onProcess: update_variant x:", update_variant);
                  debug && console.log("onProcess: current_values:", current_values.length, current_values);
                ...
                debug && console.log("current_values_string:", current_values_string, current_values_string.includes("nurseblake"))
                if (data_value.includes("nurseblake") || current_values_string.includes("nurseblake"))  {
                    $product.find('.plg-impact.plg-make-a-wish').removeClass('hide-plg-impact');
                    $product.find('.product-gallery__sheet_item .rvideo').addClass('d-none');
                }
                else {
                    $product.find('.plg-impact.plg-make-a-wish').addClass('hide-plg-impact');
                    $product.find('.product-gallery__sheet_item .rvideo').removeClass('d-none');
                }
                if (data_value.includes("lemonade") || current_values_string.includes("lemonade"))  {
                    $product.find('.plg-impact.plg-alexs-lemonade').removeClass('hide-plg-impact');
                }
                else {
                    $product.find('.plg-impact.plg-alexs-lemonade').addClass('hide-plg-impact');
                }
                debug && console.log("data_value:", data_value, "current_values_string:", current_values_string, "json:", json);
                if (handle == "antigrav1") {
                    if (data_value.includes("snow") || current_values_string.includes("snow"))  {
                        $product.find('.plg-impact.plg-laundry-truck').removeClass('hide-plg-impact');
                    }
                    else {
                        $product.find('.plg-impact.plg-laundry-truck').addClass('hide-plg-impact');
                    }
                }

                const option1 = $product.find('[data-option1="' + data_value + '"]')
                debug && console.log("variant_description:", option1);

                if (option1.length) {
                    $product.find('[data-option1]').addClass('d-none')
                    option1.removeClass('d-none')
                }
          };
          
          function onFit(e, that) {
            var $this = that || $(this),
                $options = $this.parent(".options-container");

            if ($this.hasClass('disabled')) {
                return;
            }

            var $values = $options.find('[data-js-option-fit]'),
                $product = $this.parents('[data-js-product]');
             //   json = $product.attr('data-json-product');

            debug && console.log("onFit: values:", $values);
            debug && console.log("onFit: this:", $this);
            debug && console.log("onFit: options:", $options);
            debug && console.log("onFit: product:", $product);
            debug && typeof json !== "undefined" && console.log("onFit: product json:", json);

            // const dataValue = $this.attr('data-value');
            // console.log("onFit: dataValue:", dataValue);

            $product.find('.blur-size').removeClass('blur-size');
            $values.removeClass('active');
            $this.filter('[data-js-option-fit]').addClass('active');
            const clickedCode = $this.filter('[data-js-option-fit]').attr('data-code');
            debug && console.log("clickedCode:", clickedCode);
            
            Array.from($values).forEach(v => {
                const code = v.getAttribute('data-code')
                debug && console.log("$value code:", code);
                if (code == clickedCode) {
                    $product.find('.'+code).removeClass('d-none');
                } else {
                    $product.find('.'+code).addClass('d-none');
                }
            })

            const selectedSize = $('.product-page-info .option-size [data-property="size"]:not([data-disable-auto-select]) .product-options__value.active');
            debug && console.log("onFit: selectedSize:", selectedSize);

            if (selectedSize && selectedSize.length) {
                const $section = selectedSize.parents('[data-property]');
                $section.attr('data-disable-auto-select', '');
                onProcess(e, selectedSize);
            } else {
                const fitText = $this.filter('[data-js-option-fit]')[0]?.innerText
                debug && console.log("fitText: ", fitText);
                $options?.parents('.option-size')?.find('[data-label-value]')?.text(fitText);
                $('input#size_attribute')?.val(fitText);
            }
          }

          $body.on('click', this.selectors.options + ' [data-js-option-value]', onProcess);
          $body.on('click', this.selectors.options + ' [data-js-option-fit]', onFit);

          function urlParams() {
            const url = new URL(location.href);
            const params = new URLSearchParams(url.search);
            const color = params.get('color')?.replace(/[-_+\/\\ ]/g, '').toLowerCase();
            let fit = params.get('fit')?.slice(0,1).toUpperCase();
            let size = params.get('size')?.toUpperCase();
            let debugParam = params.get('debug');
            if (debugParam) debug = true;
            const paramString = (color ? "  color: " + color : "") 
                + (fit ? "  fit: " + fit : "")
                + (size ? "  size: " + size : "");
            debug && console.log("urlParams:", paramString);
            if (size) {
                if (size.search("M") > -1 || fit == "M") {
                    fit = "M";
                    size = size.replace(/[^0-9.]/g,'');
                    const sizeM = size.replace(/[.]/g,'');
                    const sizeW = String(size - 0 + 2).replace(/[.]/g,'');
                    size = "w" + sizeW + "m" + sizeM;
                } else {
                    // if (size.search("W") > -1 || fit == "W") {
                        fit = "W";
                        size = size.replace(/[^0-9.]/g,'');
                        const sizeM = String(size - 2).replace(/[.]/g,'');
                        const sizeW = size.replace(/[.]/g,'');
                        size = "w" + sizeW + "m" + sizeM;
                    // } else {
                        // size = null; 
                    // };
            ...
            debug && console.log("updated size:", size);
            if (fit) {
                const $fit = $('[data-js-option-fit][data-code="unisex-' + fit +'"]');
                if ($fit) $fit.trigger("click");
            }
            if (size) {
                setTimeout(()=> {
                    const $size = $('.option-size .product-options__value[data-value="' + size + '"]');
                    debug && console.log("$size:", $size)
                    if ($size) $size.trigger("click");
                }, 0);
            }
            if (color) {
                setTimeout(()=> {
                    const $color = $('.option-color [data-js-option-value][data-value="' + color +'"]');
      ...
      _loadJSON: function ($product, json, callback, animate) {
        debug && console.log("_loadJSON $product:", $product, json)
        if ($product && $product[0]) {
            if($product[0].hasAttribute('data-js-process-ajax-loading-json')) {
                $product.one('json-loaded', function () {
                    if(callback) {
                        debug && console.log("_loadJSON data-json-product:", $product.attr('data-json-product'))
            ...
            if(json) {
                if(callback) {
                    //   console.log("_loadJSON json:", json)
      ...
      switchByImage: function($product, get_image, id, callback) {
        debug && console.log("switchByImage");
          var _ = this,
      ...
      _updatePossibleVariants: function ($product, data) {
          ...
          debug && console.log("product:", $product, "data:", data);
      ...
      _switchVariant: function($product, data) {
        // console.log("_switchVariant: has_unselected_options:", data.has_unselected_options);
          ...
          $.each(data.json.variants_quantity, function() {
              if(+this.id === +data.update_variant.id) {
                  if((+this.quantity <= 0 || this.preorder == "force") && this.policy == 'continue') {
      ...
      _updateContent: function($product, data) {
          ...
          if (theme.StoreLists) theme.StoreLists.checkProductStatus($product);
          if (theme.ProductImagesNavigation) theme.ProductImagesNavigation.switch($product, data);
          ...
          if($clone_product.length && theme.ProductImagesNavigation) {
      ...
      _updateAddToCart: function($product, $clone_product, data) {
          ...
          document.querySelector('.klaviyo-bis-trigger')?.classList.add('d-none');
          document.querySelector('.product-page-main .hsa-slug')?.classList.remove('v-hide');
          if($button.length && !data.has_unselected_options) {
            // console.log("_updateAddToCart: variant_pre_order:", data.update_variant.variant_pre_order);
            // console.log("data.update_variant.variant_pre_order:", data.update_variant.variant_pre_order);
              if(data.update_variant.variant_pre_order) {
                  $button.removeAttr('disabled');
                  $button.attr('data-button-status', 'pre-order');
              } else {
                  if(data.update_variant.available) {
                    ...
                    //   console.log("sold out: metafields:", window.cpvBis[data.update_variant.id] );
                    //   const urlSearch = new window.URLSearchParams(url?.search) 
                    //   if (urlSearch.get("bis") !== null && window.cpvBis[data.update_variant.id]) {
                      if (window.cpvBis[data.update_variant.id] == "true") {
                        document.querySelector('.klaviyo-bis-trigger')?.classList.remove('d-none');
                        document.querySelector('.product-page-main .hsa-slug')?.classList.add('v-hide');
                      }
                  }
              }
          }
      ...
      _updatePrice: function($product, $clone_product, data) {
          var $price = $product.add($clone_product).find('[data-js-product-price]').parent(),
              ...
              details;
        //   console.log("in updatePrice: ", $price.length, typeof theme.ProductPrice, data.update_variant.title, data, $product);
        
          if($price.length && theme && theme.ProductPrice) {
    // scott discount price display: DON'T EDIT HERE. CHANGE SETTING IN DISCOUNT-DISPLAY-ENABLE.LIQUID
            // console.log("discount_display:", window.discount_display);
            if (window.discount_display_enable 
                        && data.json.handle != "wear-gales-gift-card" 
                        && data.json.handle != "wholesale-giftable-codes" 
                        && (data.update_variant.sku.includes(window.discount_display?.sku_filter) || !window.discount_display.sku_filter)) {
                data.update_variant.compare_at_price = data.update_variant.price;
                data.update_variant.price = data.update_variant.price - parseInt(data.update_variant.price * window.discount_display.discount_percent_off / 100);
            }
    // END DISCOUNT PRICE DISPLAY CODE

            theme.ProductPrice.insertFull($price, data.update_variant);
            // console.log("in updatePrice: ", "price:", data.update_variant.price, "$price:", $price, "$details:", $details);
            // if (typeof displayDiscount == "function") displayDiscount(data.update_variant.price, $price);
          }
      ...
      _updateTextLabelValue: function($product, $clone_product, data) {
          ...
          if(data.update_variant.option1) {
              ...
              const styleLabelValue = $product[0].querySelector(".option-style [data-label-value]");
              if (styleLabelValue) {
                const styleSku = data?.update_variant?.sku?.slice(0,2).toUpperCase();                
                debug && console.log("styleLabelValue:", styleLabelValue);
                switch (styleSku) {
                    case "GP": 
                        styleLabelValue.textContent = "Pro Line";
                        break; 
                    case "GF": 
                        styleLabelValue.textContent = "Frontline";
                        break;
                }
              }
          }
    
          const option2 = data.update_variant.option2;
          if(option2) {
            let sizeText = option2.slice(0); //copy
            debug && console.log("_updateTextLabelValue: option2:", sizeText);
            const fitSelector = $('.options-container[data-fit]')
            const sizeSelector = $('.option-size .product-options__section .product-options__value.active');
            if (fitSelector && fitSelector.length) {
                sizeText = fitSelector.find(".active")[0]?.innerText
                if (sizeSelector && sizeSelector[0] && !data.has_unselected_options) {
                    const selSize = option2?.split(sizeText?.slice(0,1)?.toUpperCase())[1]?.split(" ")[0];
                    sizeText = sizeText + " " + selSize
                }
                $('input#size_attribute')?.val(sizeText);
            } else {
                sizeText = sizeText.replace(" W", " / Womens ").replace("M", " Mens "); // Expand out Mens and Womens
            }
            debug && console.log("_updateTextLabelValue: sizeText:", sizeText);
            $container.eq(1).add($clone_container.eq(1)).find('[data-label-value]').text(sizeText);
          }
      ...
      _updateAvailability: function($product, data) {
          ...
          if($availability.length) {
              ...
              // console.log("data:", data)
              if(data.update_variant.variant_pre_order) {
                html += theme.strings.availability_value_preorder;
                html = Shopify.addValueToString(html, {
                    'count': quantity,
                    'item': quantity === 1 
  ...
  class CartData {
      ...
      addItems(items) {
        //   console.log("addItems: items:", items);
          const requestParams = {
              method: 'POST',
              headers: {
      ...
      qtyMaxWarn(line) {
        // console.log("qtyMaxWarn: line:", line);
        const warnElement = document.querySelector('.quantity-max-note.product-cart-line-' + line);
        // console.log("warnElement:", warnElement);
        if (warnTimer) clearTimeout(warnTimer);
        if (warnElement) {
            warnElement.classList.add("op1");
            // console.log("warnElement: opacity:", warnElement.style.opacity);
            warnTimer = setTimeout(function() {warnElement.classList.remove("op1");}, 3000)
        }
      }
      changeItemById(id, quantity) {
          ...
          return fetch(`${this.rootURL}/change.js`, requestParams)
              .then(response => response.json())
              .then(data => {
                if (data.status != 422) this.setCurrentData(data);
                else this.qtyMaxWarn();
              })
              .catch(error => console.error('Error:', error));
      }
  ...
  class CartViewer extends CartData {
      ...
      updateCounter() {
        ...
        //   console.log("updateCounter: currentData:", this.currentData);
        //   console.log("updateCounter: count:", count);
      ...
      updateCartPopupContent() {
          if(this.$popup.length) {
              return this.getPopupContent().then(data => {
                 ...
                 if (typeof updateDiscount == "function") updateDiscount();
              });
          } else {
  ...
  class CartController extends CartViewer {
      ...
      addToCartController() {
          ...
          $body.on('click', this.selectorButtonAdd, function(e) {
            ...
            //   console.log("add to cart clicked:", $this, attrButtonStatus)
              ...
              if(attrButtonStatus !== 'added' && !$this.hasClass('active')) {
                  ...
                  if(attrButtonStatus === 'select') {
                    ...
                    //   console.log("$product:", $product)
                      $product.find('.product-options')[0]?.scrollIntoView({behavior: 'smooth'});
                    //   _.goToProductPage($product);
                  } else {
                      const $form = $this.parents('form'),
                          formData = _.serializeForm($form);

                        //   console.log("addtoCartController: formData:", formData, $form)
                          ...
                          .then(response => {
                              if(response.ok) {
                                if ($product[0]) {
                                    ...
                                    const prod = JSON.parse($product[0].dataset.jsonProduct);
                                    ...
                                    dataLayer.push({
                                        ...
                                        'ecommerce': {
                                                'items': [{
                                                'item_id': variant.featured_image ? variant.featured_image.product_id.toString() : null,
                                                'item_name': variant.name.toString(),
                                                'item_quantity': formData.quantity.toString(),
                                                'item_variant_id': variantID.toString(),
                                                'currency': Shopify.currency.active.toString(),
                                                'price': (variant.price / 100).toString(),
                                                'item_sku': variant.sku ? variant.sku.toString() : "",
                                                'item_handle': prod.handle.toString()
                                                }]
                                            }
                                    });
                                }
      ...
      changeQuantityCartPageController() {
          ...
          $body.on('change keyup custom.change', this.selectorPageInputQuantity, theme.debounce(function(e) {
                const $this = $(e.target);
                // console.log("changeQuantity: $this:", e.target, $this);
                window.ttt = e.target;
                // console.log($this.closest("[data-product-variant-id]").attr("data-product-variant-id"));

                const quantity = Math.trunc($this.val()); // || 1, allow "0" units to remove item from cart
                const line = $this.parents('[data-js-product]').attr('data-product-cart-line');
                const dataLastUpdateValue = $this.attr('data-last-update-value');
                // console.log("cart item qty:", quantity, "dataLastUpdateValue:", dataLastUpdateValue);
      ...
      statusButtonAdded($btns) {
          ...
          const saveStatus = $btns.attr('data-button-status');
        //   console.log("statusButtonAdded: saveStatus:", saveStatus);
          if (saveStatus) {
            $btns.attr('data-button-status-saved', saveStatus)
          }
          $btns.removeAttr('disabled').attr('data-button-status', 'added');
      }
  
      enableButton($btns) {
          const savedStatus = $btns.attr('data-button-status-saved');
        //   console.log("savedStatus:", savedStatus);
          if (savedStatus) {
             $btns.attr('data-button-status', savedStatus)
             $btns.removeAttr('data-button-status-saved');
          } else {
             $btns.removeAttr('data-button-status');
          }
          $btns.removeAttr('disabled style').removeClass('active')
      }
      ...
      serializeForm($form, serialize_obj = {quantity: 1}) {
        //   console.log("serializeForm: serialize_obj:", serialize_obj);
